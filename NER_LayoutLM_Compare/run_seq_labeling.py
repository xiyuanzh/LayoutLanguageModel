...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    for i in range(len(out_label_list)):
        for j in range(len(out_label_list[i])):
            true_list.append(out_label_list[i][j])
            pred_list.append(preds_list[i][j])
...
def main():  # noqa C901
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
            ...
            with open(
                ...
                for line in f:
                    if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                        ...
                        cur_line = line.split()
                        cur_pred = predictions[example_id].pop(0)
                        if(cur_line[-1] != cur_pred):
                            output_line = (
                                cur_line[0]
                                + " "
                                + cur_pred
        ...
        true_label_list = []
        pred_label_list = []
        pred_file = open(os.path.join(args.output_dir, "test_predictions.txt"), "r", encoding="utf8")
        pred_file = [line.rstrip('\n') for line in pred_file]
        true_file = open(os.path.join(args.data_dir, "test.txt"), "r", encoding="utf8")
        true_file = [line.rstrip('\n') for line in true_file]
        #assert len(pred_file) == len(true_file)
        i = 0
        for line in true_file:
            if line.startswith("-DOCSTART-") or line == "" or line == "\n" or line.startswith("<tainted>"):
                #if not pred_file[i]:
                #    i += 1
                continue
            else:
                line = line.split()
                pred_line = pred_file[i].split()
                true_label_list.append(line[-1])
                pred_label_list.append(pred_line[-1])
                i += 1

        results_taint = {
            "taint_precision": precision_score(true_label_list, pred_label_list),
            "taint_recall": recall_score(true_label_list, pred_label_list),
            "taint_f1": f1_score(true_label_list, pred_label_list),
        }

        output_test_results_file = os.path.join(args.output_dir, "test_results.txt")
        with open(output_test_results_file, "a") as writer:
            for key in sorted(results_taint.keys()):
                writer.write("{} = {}\n".format(key, str(results_taint[key])))

        print(len(pred_label_list),len(pred_list))
        print(results_taint)
