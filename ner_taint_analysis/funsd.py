    ...
    def __init__(self, args, tokenizer, labels, pad_token_label_id, mode):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            features = convert_examples_to_features(
                ...
                taint_ratio=args.train_taint_probability if mode == "train" else args.test_taint_probability,
                random_seed=args.train_taint_seed if mode == "train" else args.test_taint_seed
            )
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, label, box, actual_bbox in zip(
            ...
            if rnd.uniform(0, 1) <= taint_ratio:
                word_tokens = [tokenizer.mask_token]
